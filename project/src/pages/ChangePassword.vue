        ...
        <form action class="form-group">
          ...
          <div class="phone-check">
            <ValidationProvider name="code" rules="required" v-slot="{ errors }">
              <input type="text" name id placeholder="验证码" class="code" v-model="formData.mycode" :disabled="inputCode" />
              <span>{{ errors[0] }}</span>
            </ValidationProvider>
          ...
          <div class="form-row">
            <ValidationProvider
              name="password"
              :rules="{regex: /^[a-zA-Z]{1}([a-zA-Z0-9]|[._]){4,19}$/,required:true}"
              v-slot="{ errors }"
            >
          ...
          <button class="register" type="button" @click="submit" :disabled="invalid">重设密码</button>
        </form>
      </ValidationObserver>
...
export default {
  ...
  methods: {
    async handleCodeClick() {
      var res = await post("/be/users/sendcode", this.formData);
    ...
    async submit() {
      var res = await post("/be/users/set", this.formData);
      switch (res.data.message) {
        case "2":
          this.$toast.fail({
            message: "验证码错误",
